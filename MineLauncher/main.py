...
Copyright (C) 2022  PythonScratcher
...
if str(absolute_path).startswith("/usr/bin"):
    absolute_path = "/usr/lib/MineLauncher/"

# Make the launcher import local files
sys.path.append(absolute_path)
if os.path.exists("/usr/lib/MineLauncher/"):
    sys.path.append("/usr/lib/MineLauncher/")
...
if not os.path.exists(f"/home/{USER}/.MineLauncher/mods"):
    os.makedirs(f"/home/{USER}/.MineLauncher/mods")
...
class ConfigMineWiz(QDialog):
    """Startup configurator for MineLauncher. Based on QDialog."""
    ...
    def link_flatpak(self):
        ...
        with open(
            f"/home/{USER}/.MineLauncher-launcher/minecraft.AppImage", "w"
...
class MineLauncher(QMainWindow):
    ...
    def __init__(self):
        ...
        if not os.path.exists(
            f"/home/{USER}/.MineLauncher/config.json"
            ...
            with open(
                f"/home/{USER}/.MineLauncher/config.json", "w"
        ...
        self.setWindowTitle("MineLauncher")  # Set the window title
    ...
    def play_tab(self) -> QWidget:
        ...
        if date.today().month == 4 and date.today().day == 1:
            ...
            if random.randint(1, 1000) == 1:
                namelabel.setText("RobloxLauncher")  # a 1/100, Robloxlauncher
            else:
                namelabel.setText("MineLauncher ")  # Else, just set it normal
    ...
    def save_profile(self):
        ...
        with open(f"/home/{USER}/.MineLauncher/config.json", "w") as file:
            file.write(json.dumps(self.conf))
    ...
    def delete_config(self):
        ...
        if button == QMessageBox.Ok:
            ...
            os.remove(f"/home/{USER}/.MineLauncher/config.json")
            self.hide()
            sys.exit()
    ...
    def delete_appimage(self):
        ...
        if button == QMessageBox.Ok:
            ...
            os.remove(f"/home/{USER}/.MineLauncher/minecraft.AppImage")
            self.hide()
            sys.exit()
    ...
    def launch(self):
        ...
        if self.showlauncher.isChecked() == True:
            self.hide()
            launcher.run(
                self.env, f"/home/{USER}/.MineLauncher/minecraft.AppImage"
            ).wait()
        else:
            launcher.run(self.env, f"/home/{USER}/.MineLauncher/minecraft.AppImage")
        self.show()
...
if __name__ == "__main__":
    ...
    if os.path.exists(f"/home/{USER}/.MineLauncher/config.json"):
        with open(f"/home/{USER}/.MineLauncher/config.json") as file:
            app.setPalette(qdarktheme.load_palette(json.loads(file.read())["theme"]))
    else:
        app.setPalette(qdarktheme.load_palette(theme))

    if not os.path.exists(f"/home/{USER}/.MineLauncher/minecraft.AppImage"):
        MineWiz = ConfigMineWiz()
        MineWiz.show()
        MineWiz.exec()
        if MineWiz.filename[0] == "":
            sys.exit(-1)
        elif MineWiz.filename[0] == False:
            print("Using /usr/bin as an executable.")
        else:
            with open(MineWiz.filename[0], "rb") as appimage:
                with open(
                    f"/home/{USER}/.MineLauncher/minecraft.AppImage", "wb"
                ) as out:
                    out.write(appimage.read())
                    os.chmod(f"/home/{USER}/.MineLauncher/minecraft.AppImage", 0o755)
